...
// TrackLogin creates an entry for every login attempt
func (ms *MySQLStore) TrackLogin(UserID int64, IPAddress string) error {
	ins := "insert into LoginAttempts(UserID, LoginTime, IpAddress) values (?,?,?)"
	_, err := ms.Database.Exec(ins, UserID, time.Now(), IPAddress)
	if err != nil {
		return fmt.Errorf("error creating login entry %v", err)
	}
	return nil
}
